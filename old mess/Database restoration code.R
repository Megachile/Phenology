...
# dbListTables(gfnew)
# dbListFields(gfnew, "taxonomyalias")
dbDisconnect(gfnew)


### This code identifies all the cases where a Tephritid species and genus has been re-created after the data loss

## Fetches all genera and species in the taxonomy table that are children of Tephritidae (89)
# 
# query <- "
#     SELECT s.*, g.*
#     FROM species AS s
#     INNER JOIN speciestaxonomy AS st ON s.id = st.species_id
#     INNER JOIN taxonomy AS g ON st.taxonomy_id = g.id
#     WHERE g.id IN (
#         SELECT g.id
#         FROM taxonomy AS f
...
#     );
# "
# resultold <- dbGetQuery(gfbackup, query)
# resultnew <- dbGetQuery(gfnew, query)
# names(resultold)[which(names(resultold) == "name")[1]] <- "species_name"
# names(resultnew)[which(names(resultnew) == "name")[1]] <- "species_name"
# names(resultold)[which(names(resultold) == "id")[1]] <- "species_id"
# names(resultnew)[which(names(resultnew) == "id")[1]] <- "species_id"
# # Merge the two dataframes on the 'name' column
# merged_df <- merge(resultold, resultnew, by = "species_name", suffixes = c("_old", "_new"))
# 
# # Identify gall IDs that are only in resultnew and not in resultold
# unique_to_new <- setdiff(resultnew$id, resultold$id)
# unique_to_old <- setdiff(resultold$id, resultnew$id)
# 
# # Filter rows where id is different between the two dataframes
# differences <- merged_df[merged_df$id_old != merged_df$id_new, ]
...
#   existing_alias <- dbGetQuery(gfnew, query_check_existence)
#   
#   if (nrow(existing_alias) == 0) {
#     dbWriteTable(gfnew, "alias", aliases_to_restore[i, , drop = FALSE], append = TRUE, row.names = FALSE)
#   }
# }
# 
# 
# # Inserting the aliasspecies records into gfnew
# # Here, I'm assuming that you want to add the associations irrespective of whether the alias existed before
# dbWriteTable(gfnew, "aliasspecies", associated_aliases, append = TRUE, row.names = FALSE)
...
# 
# 
# dbGetQuery(gfbackup, "SELECT * FROM gallspecies WHERE species_id = '3945'")
# dbGetQuery(gfbackup, "SELECT * FROM gall WHERE id = '2657'")
# dbGetQuery(gfnew, "SELECT * FROM gallspecies WHERE gall_id = '2657'")
# dbGetQuery(gfnew, "SELECT * FROM gall WHERE id = '2657'")
# 
# 
# query <- "
#     SELECT g.id 
#     FROM gall AS g
#     LEFT JOIN gallspecies AS gs ON g.id = gs.gall_id
#     WHERE gs.gall_id IS NULL;
# "
# 
# # Adjusted function to fetch species_id for a given gall_id from the gfbackup
# fetch_species_id <- function(gall_id) {
#   query <- paste0("SELECT species_id FROM gallspecies WHERE gall_id = ", gall_id)
#   result <- dbGetQuery(gfbackup, query)$species_id
#   
#   # Return NA if there's no result
#   if (length(result) == 0) {
#     return(NA)
#   } else {
#     return(result)
...
# }
# 
# # Apply the function to each gall_id in unique_to_new
# species_ids <- sapply(unique_to_new, fetch_species_id)
# 
# # Now combine the gall IDs and species IDs into a dataframe
# result <- data.frame(gall_id = unique_to_new, species_id = unlist(species_ids))
# 
# # Step 1: Filter out rows with NA in species_id
# filtered_result <- result[!is.na(result$species_id), ]
# 
# # Step 2: Insert the remaining rows into the gallspecies table in gfnew
# 
# # Function to insert a row into the gallspecies table
# insert_row <- function(gall_id, species_id) {
#   query <- paste0("INSERT INTO gallspecies (gall_id, species_id) VALUES (", gall_id, ", ", species_id, ")")
#   dbExecute(gfnew, query)
# }
# 
# # Apply the function to each row of filtered_result
# mapply(insert_row, filtered_result$gall_id, filtered_result$species_id)
# 
# # Optional: Confirm that the records were added
# query_string <- paste("SELECT * FROM gallspecies WHERE gall_id IN (", paste(unique_to_new, collapse = ","), ")", sep = "")
# added_rows <- dbGetQuery(gfnew, query_string)
# print(added_rows)
# 
# 
